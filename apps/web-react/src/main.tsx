...
const hash = (value: string) => [...value].reduce((h, c) => (h * 31 + c.charCodeAt(0)) >>> 0, 0)
...
function Recorder({ onRecorded }: { onRecorded: (blob: Blob, duration: number) => void }) {
  ...
  const start = async (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    try {
      if (!navigator.mediaDevices?.getUserMedia || !('MediaRecorder' in window)) throw new Error('recording-unsupported')
      ...
      mediaRecorder.onstop = () => {
        ...
        setRecording(false)
        setElapsed(0)
        if (blob.size > 0 && duration > 0.05) onRecorded(blob, duration)
      }
      mediaRecorder.start()
      setRecording(true)
      timer.current = window.setInterval(() => setElapsed((Date.now() - started.current) / 1000), 100)
  ...
  return <div className={`recorder ${recording ? 'recording' : ''}`} onPointerDown={start} onPointerUp={stop} onPointerCancel={stop} onPointerLeave={stop}>{recording ? `吹き込み中… ${Math.ceil(elapsed)}s / 30s` : '長押しで吹き込む'}</div>
}
...
function App() {
  ...
  const addRecorded = (blob: Blob, duration: number) => {
    ...
    setVoices((current) => [...current, { id, audio_url: url, duration, device_id: myId }])
  ...
  return <main><header><button aria-label="ロビーへ">‹</button><div><strong>Voice BBS</strong><small>{status}</small></div><span className="badge">React + Vite PoC</span></header><section className="room" aria-label="Voice bubbles">{voices.length === 0 && <p className="empty">長押しで声を録音すると、泡になります</p>}{voices.map((voice) => <VoiceBubble key={voice.id} voice={voice} mine={voice.device_id === myId} playing={playing === voice.id} onPlay={() => play(voice)} onLongPress={() => setStatus('自分の声: 削除アクション')} />)}</section><footer><Recorder onRecorded={addRecorded} /><p>最大30秒。録音データはこのPoCのブラウザ内だけで再生します。</p></footer></main>
}

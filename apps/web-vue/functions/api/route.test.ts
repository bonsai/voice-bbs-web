...
function dbMock(overrides: Record<string, unknown> = {}) {
  ...
  const prepare = vi.fn(() => ({ bind: vi.fn(() => ({ first, all, run })), first, all, run }))
...
describe('Hono API', () => {
  ...
  it('GET /api/categories returns categories from D1', async () => {
    const db = dbMock(); db.all.mockResolvedValue({ results: [{ id: 'general', name: '一般' }] })
  ...
  it('GET /api/threads applies category, query and limit filters', async () => {
    const db = dbMock(); db.all.mockResolvedValue({ results: [{ id: 'thread-1', category_id: 'general' }] })
  ...
  it('GET /api/threads/:id/posts returns posts from D1', async () => {
    const db = dbMock(); db.all.mockResolvedValue({ results: [{ id: 'post-1', thread_id: 'thread-1' }] })
  ...
  it('POST /api/threads/:id/posts uploads the image and creates a post', async () => {
    const db = dbMock(); db.first.mockResolvedValue({ cnt: 0 })
    ...
    expect(body.ok).toBe(true); expect(body.id).toEqual(expect.any(String)); expect(body.url).toMatch(/^\/api\/audio\/.+\.png$/); expect(body.remaining).toBe(3)
    expect(bucket.put).toHaveBeenCalledTimes(1); expect(db.run).toHaveBeenCalledTimes(2); expect(db.all).toHaveBeenCalledTimes(1)
  })

  it('POST /api/threads/:id/posts rejects a device that reached the daily limit', async () => {
    const db = dbMock(); db.first.mockResolvedValue({ cnt: 4 }); const bucket = { put: vi.fn(), delete: vi.fn() }
    ...
    expect(response.status).toBe(429); await expect(response.json()).resolves.toEqual({ ok: false, error: 'limit_reached' }); expect(bucket.put).not.toHaveBeenCalled(); expect(db.run).not.toHaveBeenCalled()
  })

  it('POST /api/threads/:id/posts removes posts beyond the 100-post cap', async () => {
    const db = dbMock(); db.first.mockResolvedValue({ cnt: 0 }); db.all.mockResolvedValue({ results: [{ id: 'old-post', audio_url: '/api/audio/old-post.png' }] })
    const bucket = { put: vi.fn(async () => undefined), delete: vi.fn(async () => undefined) }
    ...
    expect(response.status).toBe(200); expect(db.run).toHaveBeenCalledTimes(3); expect(bucket.delete).toHaveBeenCalledWith('posts/old-post.png')
  })
  ...
  it('DELETE /api/posts/:id rejects missing device_id', async () => {
    ...
    expect(response.status).toBe(400); await expect(response.json()).resolves.toEqual({ ok: false, error: 'missing_device_id' })
  })

  it('DELETE /api/posts/:id rejects a non-owner', async () => {
    const db = dbMock(); db.first.mockResolvedValue({ device_id: 'owner-device', audio_url: '/api/audio/post-1.png' })
    ...
    expect(response.status).toBe(403); await expect(response.json()).resolves.toEqual({ ok: false, error: 'forbidden' }); expect(db.run).not.toHaveBeenCalled()
  })

  it('DELETE /api/posts/:id deletes the owner post and R2 object', async () => {
    const db = dbMock(); db.first.mockResolvedValue({ device_id: 'owner-device', audio_url: '/api/audio/post-1.png' }); const bucket = { delete: vi.fn(async () => undefined) }
    ...
    expect(response.status).toBe(200); await expect(response.json()).resolves.toEqual({ ok: true, deleted_by: 'owner' }); expect(db.run).toHaveBeenCalledTimes(1); expect(bucket.delete).toHaveBeenCalledWith('posts/post-1.png')
  })
  ...
  it('DELETE /api/posts/:id allows the configured admin token', async () => {
    ...
    expect(response.status).toBe(200); await expect(response.json()).resolves.toEqual({ ok: true, deleted_by: 'admin' })
  })

  it('GET /api/count/:device_id returns remaining quota', async () => {
    const db = dbMock(); db.first.mockResolvedValue({ cnt: 2 })
    const response = await app.request('/api/count/device-1', {}, { DB: db as never, BUCKET: {} as never, R2_PUBLIC_URL: '' })
    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toEqual({ ok: true, count: 2, remaining: 2 })
  })

  it('GET /api/count/:device_id never returns negative remaining quota', async () => {
    const db = dbMock(); db.first.mockResolvedValue({ cnt: 7 })
    const response = await app.request('/api/count/device-1', {}, { DB: db as never, BUCKET: {} as never, R2_PUBLIC_URL: '' })
    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toEqual({ ok: true, count: 7, remaining: 0 })
  })
})
